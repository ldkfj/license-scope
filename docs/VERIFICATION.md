...
- **Corrected model**: Shared pure validation functions (`validateTransactionBinding`, `reconcileRequestRecord`, `reconcileResolveRecord`, `reconcileRetryRecord`) using browser-native base64 decoding (`atob` + `Uint8Array`, zero `Buffer` dependency) bind the transaction envelope (exact hash, sender account, contract recipient, method name, payload arguments, returned assessment ID) and validate legitimate on-chain state machine progression:
  ...
  - *Unsuccessful or ambiguous receipt*: The persisted same-hash mutex is retained. Only a receipt whose every present calldata/result representation parses consistently, proves `FINALIZED` successful execution, and matches a full authoritative identity/state readback may clear it. Failed, malformed, contradictory, or duplicate transactions are never reported as successful and never unlock a replacement write.
...
|---|---|---|---|---|
| Requester creates assessment | Request form + browser wallet | `request_assessment` | Direct pending/duplicate tests; frontend finality/readback tests | Correction-specific request `0x34a4aac8...0809`: `FINALIZED / MAJORITY_AGREE`, both leaders `SUCCESS`; recorded immediate readback was assessment #2 `PENDING` with the current policy hash, exact CoSearch revision, and `COMMERCIAL_REDISTRIBUTION`. That historical PENDING snapshot is not independently reconstructible from current terminal state. Historical request: `0x9df088...93b73` |
| Resolver evaluates pending assessment | Registry resolve action | `resolve_assessment` | Direct evidence/consensus/adversarial tests; frontend terminal readback and safe terminal-failure reconciliation tests | Correction attempt `0x06942fe5...e075`: authoritative final receipt `FINALIZED / MAJORITY_DISAGREE`; recorded immediate readback `PENDING` is historical and not independently replayable. Correction success `0x2340d9a2...a430`: `FINALIZED / MAJORITY_AGREE`, both leaders `SUCCESS`; assessment #2 `BLOCK / EXPLICIT_USE_RESTRICTION`, subject/revision `EXACT`, sufficient evidence, current policy hash. Historical evidence: `0x98a473...9c562` |
