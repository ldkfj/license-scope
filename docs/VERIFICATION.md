...
- The navbar exposes an explicit top-right wallet control, EIP-6963 wallet chooser, live account/network state, change-wallet, change-account, and disconnect actions. The announcement listener remains active for the page lifetime, and delayed announcements replace the legacy chooser option while the page is open. `Connect & Sign` discovers providers without requesting an account; only the provider explicitly selected by the user receives later account, chain, signature, client, and transaction requests. The legacy `window.ethereum` provider is merely offered as a chooser option when no EIP-6963 provider has announced and is never used before selection. Provider UUIDs are not persisted because EIP-6963 does not guarantee stable UUIDs across reloads; a reload intentionally reopens the explicit selection flow. A wallet is treated as connected only after `personal_sign` confirms control for the current tab session: the recovered signer and a post-recovery `eth_accounts` readback must both match the requested account before authorization is stored. The message states that signing is gasless and submits no transaction. First connection, reconnection after disconnect, every provider selection, and every account change invalidate the previous marker and require a fresh signature. Disconnect first attempts wallet permission revocation and verifies `eth_accounts` readback. Providers that cannot revoke authorization still enter a tab-scoped disconnected state that suppresses automatic account restoration and blocks contract writes until the user explicitly selects `Connect & Sign`, chooses a provider, and signs again.
