...
- The navbar exposes an explicit top-right wallet control, live account/network state, change-account and disconnect actions. A wallet is treated as connected only after `personal_sign` confirms control for the current tab session; the message states that it is gasless and submits no transaction. First connection, reconnection after disconnect, and account changes require a fresh signature. Disconnect first attempts wallet permission revocation and verifies `eth_accounts` readback. Providers that cannot revoke authorization still enter a tab-scoped disconnected state that suppresses automatic account restoration and blocks contract writes until the user explicitly selects `Connect Wallet` and signs again.
...
4. **Dependency audit observation — REOPENED, NON-BLOCKING AT CURRENT GATE.** Current `npm audit` reports two moderate PostCSS advisories nested under Next; `npm audit --audit-level=high` exits successfully. The suggested automatic fix requires forcing Next `16.3.0` outside the exact declared version, so no unreviewed dependency mutation was performed. Unit, typecheck, lint, and production-build gates pass.
