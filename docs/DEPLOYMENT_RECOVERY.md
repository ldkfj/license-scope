...
Correction-specific assessment `#2` exercised the newly installed full-source path. Request `0x34a4aac8b2b878ab9a442ffd76b70712a3b5e973954a3876deb289f292850809` created the immutable `COMMERCIAL_REDISTRIBUTION` record under the new policy hash. The first resolve `0x06942fe5358d40520fefcf9d0431c5a352ee670b2547dc8336eebe6502a4e075` is authoritatively `FINALIZED / MAJORITY_DISAGREE`; it was transiently observed as `UNDETERMINED` before finalization. The recorded immediate readback remained `PENDING`, but that historical snapshot is not independently reconstructible from current state. The resumed second resolve `0x2340d9a261c9aff0b481b4bb4d8fe404d038bbc900f220fa2a65f130dfdda430` reached `FINALIZED / MAJORITY_AGREE`; both leader receipts succeeded, and readback returned `BLOCK / EXPLICIT_USE_RESTRICTION`, subject and revision `EXACT`, sufficient evidence, `CC-BY-NC-4.0`, retry count `0`, and the current policy hash.
...
- **Fail-closed recovery boundary**: Unsuccessful, malformed, contradictory, or otherwise ambiguous receipts retain the persisted same-hash lock. Clearing requires consistent parsing of every present calldata/result representation, successful finalized execution, and authoritative readback matching the full persisted assessment identity, policy, state transition, and bounded retry count (`0..2`).
- **Remediation**: Shared validation functions (`validateTransactionBinding`, `reconcileRequestRecord`, `reconcileResolveRecord`, `reconcileRetryRecord`) using browser-native base64 decoding bind every present transaction representation and validate state-machine progression. Failed or ambiguous Request, Resolve, and Retry receipts retain the persisted same-hash lock; only validated successful execution plus authoritative readback clears it. Resolve/Retry persistence includes the complete pre-action assessment snapshot and immutable policy binding.
...
- **Verification**: 94 unit and regression tests pass in `frontend/tests/`, including malformed mixed representations, missing leader returns, mirrored persistence failures, full snapshot binding, and bounded retries. Public Vercel E2E on release `4498815f6899bd632f3ab09037077ea5d4fb342d` reconciled the retained request hash without a new broadcast, cleared the write lock, kept it cleared after reload, and returned the exact terminal assessment and policy readback with no browser-console errors.
